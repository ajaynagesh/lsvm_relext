...
void find_most_violated_constraint_marginrescaling_all(LABEL *ybar_all, LATENT_VAR *hbar_all, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm, int numEgs, char *tmpdir, char *trainfile){
	...
	strcpy(command,"export LD_LIBRARY_PATH=/usr/lib/lp_solve && java -Xmx8G -cp java/bin:java/lib/* javaHelpers.FindMaxViolatorHelperAll ");
	strcat(command,filename);
	//strcat(command," dataset/reidel_trainSVM.data");
	strcat(command, " ");
	strcat(command, trainfile);
	//printf("Running : %s\n", command);
	system(command);
	//system("export LD_LIBRARY_PATH=/usr/lib/lp_solve && java -Xmx8G -cp java/bin:java/lib/* javaHelpers.FindMaxViolatorHelperAll "
	//		"tmpfiles/max_violator_all dataset/reidel_trainSVM.data");
...
void infer_latent_variables_all(LATENT_VAR *imputed_h, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm, int numEgs, char * tmpdir, char *trainfile){
	...
	char *command = malloc(1000);
	strcpy(command,"export LD_LIBRARY_PATH=/usr/lib/lp_solve && java -Xmx8G -cp java/bin:java/lib/* javaHelpers.InferLatentVarHelperAll ");
	strcat(command,filename);
	//strcat(command," dataset/reidel_trainSVM.data");
	strcat(command, " ");
	strcat(command, trainfile);
	//printf("Running : %s\n", command);
	system(command);
//	system("export LD_LIBRARY_PATH=/usr/lib/lp_solve && java -Xmx8G -cp java/bin:java/lib/* javaHelpers.InferLatentVarHelperAll "
